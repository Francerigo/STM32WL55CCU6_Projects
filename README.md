...
|-------------|------------------------------------------|-------------|
| **AT+TCONF** | `AT+TCONF?\r`<br>`AT+TCONF=1\r`<br>`AT+TCONF=1,0,3600\r` | **Time Synchronization Configuration**<br>- Configures network time synchronization for Class B/C<br>- `<mode>`: `0`=disable, `1`=enable auto-sync<br>- `<src>`: time source (0=network, 1=GPS)<br>- `<interval>`: sync interval in seconds<br>**Response:** `+TCONF: <mode>,<src>,<interval>` then `OK` [web:4] |
